...
def main():
    ...
    parser.add_argument(
        "--clean_audio_dir", type=str, default='data/feats/vggish/', help="audio dir")
    parser.add_argument(
    ...
    if args.mode == 'train':
        ...
        val_dataset = LLP_dataset(label=args.label_val, audio_dir=args.clean_audio_dir, video_dir=args.video_dir, st_dir=args.st_dir, transform = transforms.Compose([ToTensor()]))
        test_dataset = LLP_dataset(train=False, label=args.label_test, audio_dir=args.clean_audio_dir, video_dir=args.video_dir, st_dir=args.st_dir, transform=transforms.Compose([ToTensor()]))
        ...
        test_dataset = LLP_dataset(label=args.label_val, audio_dir=args.clean_audio_dir, video_dir=args.video_dir,
        ...
        test_dataset = LLP_dataset(label=args.label_test, audio_dir=args.clean_audio_dir, video_dir=args.video_dir,  st_dir=args.st_dir, transform = transforms.Compose([
                                               ToTensor()]))
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=1, pin_memory=True)
        model.load_state_dict(torch.load(args.model_save_dir + args.checkpoint + ".pt"))
