...
def main():
    ...
    if args.mode == 'estimate_labels':
        ...
        val_dataset = LLP_dataset(label=args.label_val, audio_dir=args.clean_audio_dir, video_dir=args.video_dir, st_dir=args.st_dir, transform = transforms.Compose([
                                               ToTensor()]))
        ...
        print('model.load_state_dict completed')
        print('len', len(train_loader))
        #train_files_ = pd.read_csv(args.label_train, sep='\t')
        #print(len(train_files_))
        get_modality_aware_label(args, model, train_loader, optimizer, criterion, epoch=1)
        
        print('.get_modality_aware_label completed')   
